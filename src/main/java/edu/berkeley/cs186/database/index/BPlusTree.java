...
public class BPlusTree {
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode start, DataBox from) {
            this.current = start;
            this.iter = (from == null) ? start.scanAll() : start.scanGreaterEqual(from);
        }   
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            while (!iter.hasNext()) {
                Optional<LeafNode> next = current.getRightSibling();
                if (!next.isPresent()) {
                return false;}
                current = next.get();
